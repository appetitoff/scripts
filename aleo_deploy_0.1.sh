...
read -p "Вставьте значение ссылки из твиттер Link: " Link
if [ -z $Link ]; then
	echo -e "${R_COL}Внимание! Значение не может быть пустым! Повторите ввод переменной!${W_COL}"
	read -p "Вставьте значение переменной Link: " Link
  exit 0
fi
RECORD=$((curl -X GET "${Link}" | jq '.execution.transitions[0].outputs[0].value'))
